        ...
        with st.sidebar:
            ...
            for log_entry in history_data['items']:
                ...
                with st.expander("View Details"):
                    ...
                    duration_str = self.format_duration(duration_seconds) if duration_seconds is not None else "N/A"
            ...
            with col1:
                if history_data['has_previous']:
                    st.markdown("<div style='text-align: center; padding-top: 5px'>", unsafe_allow_html=True)
                    if st.button('←'):
                        st.session_state['history_page'] = page - 1
                        st.rerun()
                    st.markdown("</div>", unsafe_allow_html=True)
            with col2:
                st.markdown(f"<div style='text-align: center; margin-top: 24px'>Page {page} of {history_data['total_pages']}</div>", unsafe_allow_html=True)
            with col3:
