...
class TestModelLogger:
    ...
    def test_load_history(self, tmp_path):
        ...
        for entry in test_entries:
            ...
            with open(tmp_path / f"run_log_{timestamp.strftime('%Y%m%d_%H%M%S')}.json", "w") as f:
                json.dump(entry, f)
    ...
    def test_display_run_history(self, logger, sample_log_entry, monkeypatch):
        ...
        mock_expander = type('MockExpander', (), {
            '__enter__': lambda x: None,
            '__exit__': lambda x, y, z, w: None
        })
        
        mock_column = type('MockColumn', (), {
            '__enter__': lambda x: None,
            '__exit__': lambda x, y, z, w: None
        })
...
def test_load_history(logger, temp_log_dir):
    ...
    for entry in test_entries:
        timestamp = datetime.fromisoformat(entry['run_timestamp'])
        with open(temp_log_dir / f"run_log_{timestamp.strftime('%Y%m%d_%H%M%S')}.json", "w") as f:
            json.dump(entry, f)
