...
def validate_excel_files(file_paths):
    """
    Validate downloaded Excel files with flexible column name matching, returning a
    dictionary with file_key as keys.
    ...
    required_column_variations = {
        ...
        "age_at_entry": ["age", "member_age", "client_age", "age_at_entry"],
        "sex": ["gender", "sex", "member_gender", "client_gender"],
        "policy_term": ["term", "policy_term", "coverage_term", "duration"],
    }
...
def upload_to_s3(content, s3_url):
    ...
    try:
        ...
        s3_client.put_object(Bucket=bucket, Key=key, Body=content_bytes)
